...
- `ProxyService` validates methods/paths, synthesizes identity headers, rejects unsafe URLs (SSRF defense), and forwards via Axios with the prebuilt HTTPS agent. Circuit breaker (failure counts, open/half-open states) and retry logic are parameterized via env.
...
### 4.6 Gateway Controller
- Handles every inbound request via `@All('*')`.
...
## 5. Request Lifecycle
1. **Ingress**: `GatewayController.handleRequest()` logs request ID/method/path; rate limiting and Helmet have already run at the Express layer.
...
- SSRF prevention through path validation and hostname blocking (private ranges, localhost, `.internal`, etc.).
...
Terminal 2: npx ts-node microservices/users-service/main.ts
Terminal 3: npx ts-node microservices/orders-service/main.ts
Terminal 4: npx ts-node microservices/permissions-service/main.ts
```
...
### 9.5 Sample Request
1. Generate a JWT with `JWT_SECRET` (e.g., `a-string-secret-at-least-256-bits-long`) and payload `{ "userId": "test-user", "roles": ["user"], "sessionId": "session123" }`.
2. Call the gateway:
```bash
